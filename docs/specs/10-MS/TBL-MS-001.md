...
**테스트 관점** 아홉 매핑이 [[TBL-DOM-003#vehicle_measure]]의 `measure_type` 아홉과 글자까지 같은지. 실 도매와 도매(공식)이 서로 다른 값으로 갈리는지(미주 누계 상세 행 기준으로 실 도매 664,269와 도매(공식) 677,201로 갈려 차이가 12,932대다. 둘을 한 값으로 합치면 이 차이가 사라진다).
...
**처리** 총계 행을 **지우지 않고** `is_total_row=true`로 표시한 뒤 그 수를 함께 돌려준다. 판정 대상에서만 뺀다. 지우면 나중에 원본과 대조할 수 없고, 남겨 두고 세지 않으면 이중 계상이 된다. **총계 행은 상세 행과 범위가 다를 수 있다.** 그 파일의 차원 범위를 총계 행이 따른다고 가정하지 않고, 상세 행과 함께 더하지 않으며, 따로 보관해 대조에만 쓴다.

**테스트 관점** 생산 파일에서 `progressRate`에 값이 있는 행이 전부 `is_total_row=true`인지(개별 행은 전부 0이다). 집계가 `is_total_row=false`인 행만 더하는지. CDO_sales_3에서 상세 행 합은 선적 679,551인데 같은 파일의 총계 행은 1,710,715이라 총계 행이 상세 행 합의 약 2.5배다. 총계 행을 상세 행과 함께 더하면 값이 두 배 넘게 부푼다. 상세 행 대권역이 전부 미주인데도 총계 행은 더 넓은 범위를 담고 있으므로, 총계 행 값을 그 파일의 차원 범위 값으로 읽지 않는지도 함께 본다.

근거: [[TBL-DOM-003#vehicle_measure]] · [[TBL-SEQ-001#SEQ-14]] · [[TBL-RFQ-001#Q65]]

...
- 문장 텍스트는 강조 토큰 `[[hl]]…[[/hl]]`과 각주 `[^n]`을 포함한 원문. 렌더는 화면 몫.

## 2. 에러

RFC 9457 `application/problem+json`. `type`은 `urn:tbl:intel:{code}`.
...
| 422 | schema-mismatch | 컬럼 대조 실패. `detail`에 다른 컬럼 목록 |
| 422 | config-out-of-range | 설정 범위 밖. `detail`에 허용 범위 |
        ...
        application/json:
          schema:
            ...
            properties:
              ...
              overwrite: { type: boolean, default: false, description: 같은 기준일 존재 시 필수 true }
    responses:
...
/api/admin/intel/ingest/runs:
  get:
    ...
    responses:
      '200': { content: { application/json: { schema: { type: object, properties: { items: { type: array, items: { $ref: '#/components/schemas/IngestRun' } }, nextCursor: { type: string, nullable: true } } } } } }
```
...
/api/admin/intel/batch/runs:
  get:
    parameters:
      ...
      - { name: cursor, in: query, schema: { type: string } }
    responses:
      '200': { content: { application/json: { schema: { type: object, properties: { items: { type: array, items: { $ref: '#/components/schemas/BatchRun' } }, nextCursor: { type: string, nullable: true } } } } } }
```
...
/api/admin/intel/batch/runs/{runId}/publish:
  post:
    responses:
      '200': { content: { application/json: { schema: { type: object, properties: { briefingId: { type: integer }, asOfDate: { type: string, format: date }, version: { type: integer } } } } } }
```
...
/api/admin/intel/master/{name}:
  get:
    ...
    responses:
      '200': { content: { application/json: { schema: { type: object, properties: { version: { type: integer }, rows: { type: array, items: { type: object } }, history: { type: array, items: { $ref: '#/components/schemas/MasterVersion' } } } } } } }
```
...
/api/admin/intel/master/gaps:
  get:
    ...
    responses:
      '200': { content: { application/json: { schema: { type: array, items: { $ref: '#/components/schemas/MappingGap' } } }, text/csv: {} } }
```
...
/api/admin/intel/config:
  get:
    responses:
      '200': { content: { application/json: { schema: { type: object, properties: { current: { $ref: '#/components/schemas/ThresholdConfig' }, ranges: { type: object }, history: { type: array, items: { $ref: '#/components/schemas/ThresholdConfig' } } } } } } }
```
  ...
  schemas:
    BriefingView:
      ...
      properties:
        ...
        indicatorBar:
          type: array
          items: { type: object, properties: { identifier: { type: string }, label: { type: string }, value: { type: number, nullable: true }, changePct: { type: number, nullable: true }, asOf: { type: string, format: date }, stale: { type: boolean } } }
        headline: { $ref: '#/components/schemas/Claim' }
        headlineBadge: { type: string }
        domainStatus:
          type: array
          items: { type: object, properties: { domain: { type: string, enum: [production, sales, inventory] }, signal: { type: string, enum: [RED, YELLOW, MONITOR] }, claims: { type: array, items: { $ref: '#/components/schemas/Claim' } }, dataQualityNote: { type: string, nullable: true } } }
        watchlist: { type: array, items: { $ref: '#/components/schemas/WatchlistCard' } }
        evidence: { type: array, items: { $ref: '#/components/schemas/Evidence' } }
    Claim:
      type: object
      properties: { seq: { type: integer }, text: { type: string, description: 강조 토큰·각주 포함 }, footnotes: { type: array, items: { type: integer } } }
    WatchlistCard:
      type: object
      properties:
        iso3: { type: string }
        ...
        structuredSignal: { type: object, properties: { metric: { type: string }, value: { type: number, nullable: true }, changePct: { type: number, nullable: true }, method: { type: string, enum: [YoY, MoM, none] } } }
        inventorySignal: { type: object, nullable: true, properties: { voyage: { type: integer }, waiting: { type: integer }, ratio: { type: number } } }
        event: { type: object, properties: { eventId: { type: integer }, title: { type: string }, category: { type: string }, severity: { type: number }, articleCount: { type: integer } } }
    ...
    Evidence:
      ...
      properties:
        ...
        display: { type: object, description: 제목·출처·게시일·URL 또는 값·기준일 }
    DetailView:
      type: object
      properties:
        iso3: { type: string }
        countryName: { type: string }
        table: { type: array, items: { type: object, properties: { iso3: { type: string }, countryName: { type: string }, wholesaleMtd: { type: integer }, wholesaleCmp: { type: integer, nullable: true }, changePct: { type: number, nullable: true }, method: { type: string } } } }
        ...
        evidence: { type: array, items: { $ref: '#/components/schemas/Evidence' } }
        indicators: { type: array, items: { type: object, properties: { identifier: { type: string }, label: { type: string }, value: { type: number }, changePct: { type: number }, asOf: { type: string, format: date } } } }
    Preflight:
      type: object
      properties:
        stagingId: { type: string }
    ...
    BatchRunDetail:
      allOf:
        ...
        - type: object
          properties:
            steps: { type: array, items: { type: object, properties: { step: { type: integer }, name: { type: string }, status: { type: string }, durationSec: { type: integer }, counts: { type: object }, error: { type: string, nullable: true } } } }
            diff: { type: array, nullable: true, items: { type: object, properties: { iso3: { type: string }, field: { type: string }, before: {}, after: {}, cause: { type: string, enum: [mapping, threshold, late_data, unknown] } } } }
    MappingGap:
      type: object
    ...
    MasterDiff:
      ...
      properties:
        stagingId: { type: string }
        sheets: { type: array, items: { type: object, properties: { master: { type: string }, added: { type: integer }, changed: { type: integer }, removed: { type: integer }, removedValues: { type: array, items: { type: string } }, affectedKeys: { type: array, items: { type: string } } } } }
    MasterVersion:
      type: object
...
- [ ] 토큰 전달 방식: 쿼리 `token`만 쓸지 헤더도 허용할지. 가정: 둘 다

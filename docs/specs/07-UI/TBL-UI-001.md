...
**둘째, 신호등은 색만으로 구분하지 않는다.** 점이나 칩에 반드시 텍스트를 병기한다. RED는 "확인 필요", YELLOW는 "주의", 신호등 없음은 "원인 미확인"이다. 색각 이상과 흑백 인쇄에서 같은 정보가 읽혀야 한다.
...
<main data-screen="UI-10" class="page" style="width:1280px;padding:28px 32px 40px">
  ...
  <section class="domain-status">
    ...
    <div class="grid-3">
      <article data-el="7" class="domain-card"><!-- 생산 --></article>
...
| 2 | 기준일과 소스 최신일 | 텍스트 | 리포트 기준일, 완성차·뉴스·시장 각각의 데이터 최신일 | 없음 |
...
| 7 | 도메인 상태 카드 | 카드 3 | 신호등 점과 상태 텍스트, 종합 관점 문장, 하단에 판정 출처와 기여 상위 | 각주는 근거 강조 |
...
3. H가 도메인 상태 접기를 눌러 3카드를 접는다. 다음 방문에도 접힌 채로 열린다.
...
1. 카드 머리에 "생성 실패, 판정값만 표시" 띠가 붙는다.
2. 설명 자리에 템플릿 한 줄이 들어간다. 신호등 "확인 필요", 후보 목록, 근접도 값은 평소와 같다.
...
2. 요약이 누적 달성 86.1%와 미달 280,809대를 말한다.
...
<div data-screen="UI-8" class="report" style="width:1180px;padding:24px;display:flex;gap:20px">
  ...
  <aside data-el="1" class="side" style="width:248px">
    ...
    <section data-el="3"><span class="lbl">지금 만들 수 있는 것</span>
      <ul><li class="active">유통 체류 <span class="num">34,104</span></li>
    ...
    <section data-el="4"><span class="lbl">없는 것</span>
      <ul class="x-list"><li>항해중 재고</li><li>선적대기 재고</li>
        <li>법인재고와 딜러재고 구분</li><li>국가별 재고 시계열</li></ul>
    </section>
  ...
  <main class="sheet">
    ...
    <div class="sheet-body">
      ...
      <section data-el="9"><span class="lbl">유도 지표 · 유통 체류</span>
        ...
        <div class="grid-3"><div class="kpi">도매 누계</div><div class="kpi">소매 누계</div><div class="kpi">차이</div></div>
      </section>
...
| 3 | 지금 만들 수 있는 것 | 목록 3 | 유통 체류, 재고 회전 MOS, 현지재고 스냅샷과 각각의 한계 | 없음 |
| 4 | 없는 것 | X표 목록 4 | 항해중, 선적대기, 법인·딜러 구분, 국가별 시계열 | 없음 |
| 5 | 버전 목록 | 목록 | 현재와 직전 | 그 버전 지면 |
| 6 | 지면 헤더 | 44px 줄 | 배지, 제목, 버전 알약, "유도 지표로만 구성" | 없음 |
| 7 | 내려받기 | 버튼 | 없음 | 파일로 |
| 8 | 원천 부재 배너 | 경고색 배너 | 왜 자기 데이터가 없는지, 무엇을 대신 쓰는지 | 없음 |
| 9 | 유통 체류 요약 | 문단과 카드 3 | 도매 누계, 소매 누계, 차이와 비율 | 없음 |
...
- 못 만드는 지표를 사이드에 목록으로 보여 준다. 항해중 재고, 선적대기 재고, 법인재고와 딜러재고 구분, 국가별 재고 시계열 넷이다([[TBL-PRD-001#R29]] 넷째 인수 기준).
...
3. 유통 체류 34,104대와 국가별 비중이 나온다. 칠레 25.2%가 가장 크다.
...
<div data-screen="UI-9" class="report" style="width:1180px;padding:24px;display:flex;gap:20px">
  ...
  <aside data-el="1" class="side" style="width:248px">
    ...
    <section data-el="3"><span class="lbl">트래킹 지표</span>
      <ul><li class="active">도매 대비 소매 <span class="num">-5.0%</span></li>
    ...
    <section data-el="4"><span class="lbl">분해 차원</span>
      ...
      <div data-el="5" class="warn-box">국가와 차종이 함께 있는 파일은 미주 한 장뿐입니다. 전 세계 파일은 실적이 비어 있습니다.</div>
    </section>
    <section data-el="6"><span class="lbl">버전</span>
      <ul><li class="active num">v12 · 현재</li><li class="num">v11 · 08-19</li></ul>
    </section>
    ...
    <div class="sheet-body">
      ...
      <section data-el="10"><span class="lbl">단계별 흐름 · 누계 실적</span>
        <div class="flow">
          ...
          <div class="gap"><span class="num">-2,350</span></div>
          <div class="stage">도매 <span class="num">677,201</span></div>
          <div class="gap gap-warn"><span class="num">-34,104</span></div>
          <div class="stage">소매 <span class="num">643,097</span></div>
        </div>
        <p class="cap">선적과 도매는 거의 붙어 있고 벌어지는 구간은 도매와 소매 사이입니다. 별도 재고 데이터가 없어 이 값으로 대신합니다.</p>
      </section>

      <section data-el="11" class="breakdown">
        <div class="sec-head"><span class="lbl">변동을 대시보드 안에서 분해</span>
          <span class="cap">도매 대비 소매 격차율 · 도매 1,000대 이상 14개국</span></div>
...
| 3 | 트래킹 지표 | 목록 3 | 도매 대비 소매, 계획 대비 진도율, 전년 동월 대비 | 선택한 지표가 본문 기준 |
| 4 | 분해 차원 칩 | 칩 다수 | 국가 29, 차종그룹 37, 대리점, 차급 16, CBU/CKD | 없음 |
| 5 | 범위 경고 | 주의색 박스 | 국가와 차종이 함께 있는 파일이 미주뿐이라는 사실 | 없음 |
| 6 | 버전 목록 | 목록 | 현재와 직전 | 그 버전 지면 |
| 7 | 지면 헤더 | 44px 줄 | 배지, 제목, 버전 알약, "미주 29개국 · 누계 기준" | 없음 |
| 8 | 내려받기 | 버튼 | 없음 | 파일로 |
| 9 | 요약 | 16px 문단 | 격차 대수와 상위 국가, 반대 방향 국가 | 각주 강조 |
| 10 | 단계별 흐름 | 3칸과 간격 2 | 선적, 도매, 소매의 누계와 구간 차이 | 없음 |
| 11 | 국가별 격차 | 막대 표 두 묶음 | 쌓이는 쪽과 덜어내는 쪽. 비율과 대수 | 없음 |
...
- 단계별 흐름의 구간 차이는 음수도 정상 값이다. 방향을 색과 부호로 함께 표시한다([[TBL-PRD-001#R30]], [[TBL-DOM-001#SalesStageFlow]]).
- 국가별 표를 "쌓이는 쪽"과 "덜어내는 쪽" 두 묶음으로 나눈다. 음수를 한 표에 섞지 않는다.
...
- 도매 정본이 정해지기 전에는 도매(공식)를 기본으로 쓰고 실 도매와의 차이를 고정 문구에 적는다([[TBL-DOM-001#ThresholdSetting]]).
- 미주 밖 국가는 실적이 비어 있어 목록에 넣지 않는다. 범위를 헤더와 사이드에 적는다.
...
1. H가 판매 대시보드를 연다.
2. 단계별 흐름에서 도매와 소매 사이가 34,104대로 벌어진 것을 본다.
...
- **신호등은 색과 텍스트를 함께.** RED "확인 필요", YELLOW "주의", 없음 "원인 미확인"이다.
...
| 바뀌면 | 어디가 바뀌나 |
|:--|:--|
| 생산에 목적지 국가가 들어온다 | [[#UI-10]] 변동 목록에 생산 카드가 오른다. [[#UI-7]] 사이드 경고가 내려간다 |
| 재고 원천이 들어온다 | [[#UI-8]] 배너와 "없는 것" 목록이 내려가고 점선 블록이 실측값으로 찬다. [[#UI-10]] 재고 카드의 유도 표기가 사라진다 |
| 미주 밖 국가의 실적이 들어온다 | [[#UI-9]] 범위 문구가 바뀌고 [[#UI-10]] 변동 목록의 국가 수가 늘어난다 |

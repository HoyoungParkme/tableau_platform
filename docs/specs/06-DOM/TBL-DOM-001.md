...
도매 정본 구분을 속성으로 둔 이유는 도매가 두 기준으로 들어오기 때문이다. **미주 누계 상세 행 기준으로 실 도매 664,269대와 도매(공식) 677,201대가 12,932대 벌어진다**(도매(공식) 대비 1.9%, [[TBL-RFQ-001#Q63]]). 이 12,932대는 미주 29개국 상세 행만 더한 값의 차이이며 이 개념이 다루는 구간과 범위가 같다. 같은 파일의 전체 총계 행에서는 실 도매 1,692,227대와 도매(공식) 1,706,430대로 차이가 14,203대이지만, 총계 행은 상세 행과 범위가 다르므로([[TBL-RFQ-001#Q65]]) 상세 행 합과 더하지 않고 대조에만 쓴다. 어느 쪽으로 계산했는지를 행에 남기지 않으면 같은 국가의 체류율이 설정에 따라 조용히 달라진다.
...
감지 단위를 속성으로 둔 이유는 세부 차종 단위 감지가 오탐을 만들기 때문이다. 2025년 11월 전년 동월 대비로 전년 판매가 0보다 컸는데 올해 0이 된 세부 차종은 팰리세이드 LX2 2,282대에서 0대, 넥쏘 FE 111대에서 0대, 니로 DE PBV 1대에서 0대 셋이며 셋 다 후속 모델 전환이다([[TBL-RFQ-001#Q64]]). 세부 차종 단위로 전년 대비를 돌리면 셋 다 -100% 급락으로 잡힌다.

감지 단위를 차종 그룹으로 두는 이유는 단위를 잘못 잡으면 같은 데이터에서 부호가 뒤집히기 때문이다. 팰리세이드 LX2는 연간으로 2024년 20,967대에서 2025년 1,403대로 93% 줄었다. 그러나 팰리세이드를 차종 그룹으로 묶으면 같은 기간 20,967대에서 55,291대로 2.6배 늘었다(LX2 1,403 + LX3 20,026 + LX3 HEV 33,862). 세부 차종만 보면 급락이고 차종 그룹으로 묶으면 급증이므로, 감지 단위를 잘못 잡으면 성장하는 차종을 급락으로 보고하게 된다([[TBL-RFQ-001#Q64]]). 기본값은 차종 그룹이고, 후속 모델 매핑 표가 들어오면 세부 차종 단위를 다시 본다([[TBL-PRD-001#R8]], PRD 6.15).
...
정본 선택 셋을 여기에 둔다. **계획이 두 종류이고 도매가 두 기준이다.** 생산 누적에서 운영계획 1,754,150대와 사업계획 2,020,531대가 266,381대 벌어지고, 미주 누계 상세 행 기준 판매에서 실 도매 664,269대와 도매(공식) 677,201대가 12,932대 벌어진다([[TBL-RFQ-001#Q63]]). 둘 다 저장하되 계산에 쓸 쪽은 하나여야 하고, 그 선택이 계획 대비 달성률과 체류율을 통째로 바꾼다. 코드에 상수로 박으면 바뀔 때 과거 판정과의 차이를 설명할 수 없으므로 설정에 두고 버전으로 남긴다. 기본값은 사업계획과 도매(공식)이다([[TBL-UC-001#UC-A1]] 4a).
...
판정 근거: 근접도 값 넷은 전부 셀 수 있는 값이라 화면에 그대로 적고 왜 이 순서인지 보여 줄 수 있다. **정렬 열쇠는 넷이다.** 날짜 차이 오름차순, 같으면 출처 수 내림차순, 그다음 기사 수 내림차순, 셋까지 같으면 후보 식별자 사전순이다. 넷째 열쇠까지 계약으로 두어야 동률에서 순서가 흔들리지 않고 같은 입력에 같은 번호가 나온다. 다만 **화면에 적는 정렬 규칙 문장은 앞의 셋만 쓴다.** 넷째는 동률을 가르는 장치이지 읽는 사람에게 설명할 기준이 아니기 때문이다. 이 값이 정렬과 신호등([[#WatchItem]])의 유일한 입력이며 모델은 손대지 못한다.
...
판정 근거: 신호등은 규칙 산출물이다. **신호등 값은 red · yellow · none · notApplicable 넷이다.** 사건 후보가 기사 수·출처 수·시간창 기준을 채우고 완성차 노출이 확인되면 red, 노출이 미확인이면 yellow, 기준에 못 미치면 none으로 목록에는 남는다. 판정 대상이 아닌 경우는 notApplicable("판정 대상 아님")이다. 근거를 함께 저장해 화면에서 왜 그 값인지 보여 준다. LLM을 꺼도 같은 값이 나온다.
...
속성: 소스(완성차 생산·완성차 판매·뉴스·블룸버그·Marklines·크로스워크), 데이터 형태(A: IF 원장, B: 피벗 리포트), 스키마 버전, 파일 기준일, 그 파일에 들어 있던 기간 구분 목록, 지표 종류 목록, 행수, 결측률, 매칭률, 중복률, 회귀 급변 여부, 총계 행 분리 보관, 원본 보관 경로, 적재 회차, 적재 시각.
...
**총계 행은 상세 행과 나눠 보관한다.** 피벗 리포트의 전체 총계 행은 그 파일의 상세 행과 범위가 다르다. 판매 샘플의 상세 행은 대권역이 전부 미주여서 선적 679,551인데 같은 파일의 총계 행은 1,710,715을 들고 있어 약 2.5배다([[TBL-RFQ-001#Q65]]). 생산 쪽에서도 진도율이 상세 행에서 전부 0이고 총계 행에만 값이 있었다([[TBL-RFQ-001#Q60]]). 그래서 적재는 총계 행을 상세 행과 더하지 않고, 총계 행이 그 파일의 차원 범위를 따른다고 가정하지도 않는다. 따로 보관해 대조에만 쓴다. 이 규칙이 깨지면 실 도매와 도매(공식)의 차이 같은 값이 그대로 부풀려진다.

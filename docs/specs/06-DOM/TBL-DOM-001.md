...
인입 샘플 실측([[TBL-RFQ-001#Q59]]~[[TBL-RFQ-001#Q64]])이 이 모델의 시간 축과 재고 자리를 바꿨다. 시간 축은 기준일자 하나가 아니라 기간 구분(일·월·누계·년)과 파일 기준일 둘이 되었고, 재고는 원천이 없어 판매의 단계 차이에서 유도하는 개념이 새로 생겼다. 데이터 형태가 둘이라는 사실도 개념 하나를 새로 만들게 했다.
...
이번 개정에서 새로 넣은 것은 `SalesStageFlow`와 `IngestFile` 둘이다. 앞의 것은 재고 원천이 없어 생긴 자리이고, 뒤의 것은 완성차 데이터 형태가 둘이라 생긴 자리다. 나머지 22개의 ID는 그대로 두고 속성과 판정 근거만 고쳤다.
...
`IngestFile`이 왼쪽 끝에서 다섯 갈래로 갈라지는 것이 이번 개정의 첫 변화다. 완성차 데이터의 형태가 둘이고 형태마다 시간 축이 달라, 어느 파일에서 온 값인지를 모르면 그 값의 기간 단위를 알 수 없다.

`SalesStageFlow`가 `Anomaly`로 직접 가는 선이 두 번째 변화다. 재고 원천이 없어 재고 변동이 판매 데이터에서 유도돼 나온다([[TBL-PRD-001#R30]]).

`CauseCandidate`가 `Event`·`MarketPoint`·`Anomaly` 셋을 가리키는 것은 그대로다. 원인 후보는 뉴스일 수도, 시장 지표일 수도, 같은 국가의 다른 도메인 변동일 수도 있다.
...
축 종류를 속성으로 둔 이유가 이번 개정의 핵심이다. **A1 생산 판정은 국가 축으로 오지 않는다.** 인입 데이터에 목적지 국가 컬럼이 없어 국내·해외와 내수·수출만 있고, 수출이 어느 나라로 가는지가 없다([[TBL-RFQ-001#Q60]]). 그래서 A1은 생산 법인과 공장 단위로만 판정이 오고, 원장 보완 집계로도 국가 축을 만들 수 없다. 축 종류가 공장인 판정은 국가 카드에 오르지 못하고 신호등 판정([[#WatchItem]])에서도 제외된다. 목적지 국가 컬럼이 들어오면 그때 국가 축으로 바뀐다.
...
속성: 국가, 기간 구분(일·월·누계·년), 파일 기준일, 선적·실 도매·도매(공식)·소매, 재고 4종, 재고 출처 구분(실측·유도), CBU 비중, 사건 수, 최대 영향도, 시장지표 as-of 값, 보완 집계 여부, 품질 플래그.
...
속성: 국가, 기간 구분, 파일 기준일, 선적, 실 도매, 도매(공식), 소매, 법인 단계 차이와 그 비율(선적 대비 도매), 딜러 단계 차이와 그 비율(도매 대비 소매), 도매 정본 구분(실 도매·도매(공식) 중 계산에 쓴 것), 산출 구분(유도·실측).
...
구간을 둘로 나눈 것은 실측 때문이다. 미주 누계에서 선적 679,551, 도매 677,201, 소매 643,097이고 앞 구간은 거의 붙어 있는 반면 뒤 구간이 34,104대(5.0%)로 벌어졌다. 국가별로는 칠레 25.2%, 페루 21.2%가 크고 두 나라는 선적과 도매가 정확히 같아 물량이 딜러 단계에서 멈춰 있다. 그래서 신호로 쓸 값은 법인 단계 차이가 아니라 딜러 단계 차이다. 다만 앞 구간도 캐나다 8.3%처럼 벌어지는 국가가 있어 둘 다 저장한다.
...
속성: 차종 코드, 국가, 구분(CBU·CKD), 취득 경로(컬럼 직독·생산 유도), 유도 비율, 신뢰도, 산출 기준 기간.
관계: 국가·기간 팩트의 CBU 비중 계산에 쓰인다.

판정 근거: **형태 B는 유도하지 않는다.** 인입 샘플의 판매 파일(판매3·5·7)에 `CBU/CKD` 컬럼이 그대로 있어 읽기만 하면 된다. 앞선 보고에서 "판매에 이 구분이 없어 생산에서 유도한다"고 적은 것은 판매1·2만 보고 내린 결론이었고 실측으로 뒤집혔다([[TBL-RFQ-001#Q62]]).

유도가 필요한 것은 형태 A(IF 원장)다. IF 판매 레이아웃 23열에 이 구분이 없어 생산 모델코드로 유도해야 한다. 그래서 취득 경로를 속성으로 두고 어느 경로로 얻었는지를 행에 남긴다. 유도 실패면 "노출 미확인"이고, 그 국가는 신호등이 RED까지 올라가지 못한다. [[TBL-PRD-001#R10]] · [[TBL-UC-001#UC-S8]]
...
**기준일 하나를 기간 구분과 파일 기준일 둘로 바꿨다.** 인입 샘플에 행 단위 기준일자가 없어 "9월 3일의 변동"을 만들 수 없다. 만들 수 있는 것은 "이 파일 기준일의 누계 기준 변동"이다. 두 값을 함께 들고 있어야 같은 국가의 월 변동과 누계 변동이 한 화면에서 구분되고, 비교 값이 어느 기간과 맞붙은 것인지 비교 기간으로 확인된다. 형태 A가 들어오면 기간 구분이 "일"이 되고 파일 기준일이 곧 기준일자다. 두 형태가 섞이면 일자를 기준으로 삼고 형태 B를 그 기준일에 붙인다([[TBL-PRD-001#R7]], PRD 6.7).
...
이번 개정에서 정본 선택 셋을 여기에 넣었다. **계획이 두 종류이고 도매가 두 기준이다.** 생산 누적에서 운영계획과 사업계획이 1.2% 벌어지고(총계 328,312 대 324,512), 판매 누계에서 실 도매와 도매(공식)이 27,135대 벌어진다([[TBL-RFQ-001#Q63]]). 둘 다 저장하되 계산에 쓸 쪽은 하나여야 하고, 그 선택이 계획 대비 달성률과 체류율을 통째로 바꾼다. 코드에 상수로 박으면 바뀔 때 과거 판정과의 차이를 설명할 수 없으므로 설정에 두고 버전으로 남긴다. 기본값은 사업계획과 도매(공식)이다([[TBL-UC-001#UC-A1]] 4a).
...
판정 근거: 같은 기준일에 여러 버전이 생길 수 있고 게시본은 하나다. 재생성이 기존 게시본을 덮지 않는다. 데이터 최신일은 파일 기준일 중 가장 늦은 것이며, 리포트 기준일과 다를 수 있어 따로 둔다. [[TBL-UC-001#UC-A4]] · [[TBL-PRD-001#R13]]
...
판정 근거: **개념으로 둔다.** 처음에는 적재를 절차로만 보고 개념을 만들지 않았는데, 실측으로 형태가 둘이라는 것이 확인되면서 값 하나의 뜻이 어느 파일에서 왔는지에 따라 달라지게 됐다. 형태 A의 행은 기준일자를 스스로 들고 있지만 형태 B의 행은 파일이 붙여 준 기준일만 가진다([[TBL-PRD-001#R1]]). 이 사실을 값 쪽에 복사해 두는 것만으로는 부족하다. 재적재 범위가 형태마다 다르기 때문이다. 형태 A는 기준일자 단위로 멱등이고 형태 B는 파일 기준일 단위로 통째 덮어쓴다. 어느 단위로 덮을지는 파일을 알아야 정해진다.
...
판정 근거: 기준일과 실행 식별자 두 축이 있어야 "언제 데이터인가"와 "언제 계산했는가"를 구분한다. 여기에 파일 기준일이 더해져 축이 셋이 됐다. 데이터가 말하는 시점(파일 기준일), 리포트가 서 있는 시점(기준일), 계산한 시점(실행) 셋이 전부 다를 수 있다. [[TBL-INFRA-001#C11]] · [[TBL-PRD-001#R24]]
...
- [ ] [[#SalesStageFlow]]의 법인 단계 구간(선적 대비 도매)을 개념에 남길지. 실측에서 대부분 국가가 0이고 캐나다·브라질만 값이 있다. 지금은 남겨 두었다

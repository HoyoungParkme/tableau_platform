...
도매 정본 구분을 속성으로 둔 이유는 도매가 두 기준으로 들어오기 때문이다. **전사 누계에서 실 도매 2,356,496대와 도매(공식) 2,383,631대가 27,135대 벌어진다**([[TBL-RFQ-001#Q63]]). 이 27,135대는 전사 누계의 차이이며 미주 구간 값이 아니다. 미주 구간의 실 도매 누계는 실측이 없어 구간 단위 차이를 적지 않는다. 어느 쪽으로 계산했는지를 행에 남기지 않으면 같은 국가의 체류율이 설정에 따라 조용히 달라진다.
